...
const WHY_INTERESTED_OPTIONS = {
  ...
  SPECIFIC_INTEREST:
    "There is something specific about {schoolName} that I'm interested in",
  OTHER: "Other",
} as const;
...
const VOLUNTEER_OPPORTUNITY_OPTIONS = {
  ...
  GARDENING: "Gardening",
  OTHER_SKILLS:
    "Other skill that I'd like to contribute (music, athletics, yoga, mindfulness, etc.)",
  BEHIND_SCENES:
    "Behind-the-scenes support for the school (website, fundraising, translation, etc.)",
...
const VolunteerSignupModal: React.FC<VolunteerSignupModalProps> = ({
  ...
  const getDisplayText = (
    option: keyof typeof WHY_INTERESTED_OPTIONS,
    schoolName: string,
  ) => {
  ...
  const handleOpportunityChange = (
    value: keyof typeof VOLUNTEER_OPPORTUNITY_OPTIONS,
  ) => {
    setFormData((prev) => {
          ...
          {Object.entries(WHY_INTERESTED_OPTIONS).map(([key, text]) => (
            <label key={key} className="flex cursor-pointer items-center gap-3">
              <input
                ...
                onChange={(e) =>
                  updateFormData("whyInterested", e.target.value)
                }
                className="text-blue-500"
              />
              <span>
                {getDisplayText(
                  key as keyof typeof WHY_INTERESTED_OPTIONS,
                  school.name,
                )}
              </span>
            </label>
            ...
            <label key={key} className="flex cursor-pointer items-center gap-3">
              <input
                ...
                checked={(formData.opportunities || []).includes(
                  key as keyof typeof VOLUNTEER_OPPORTUNITY_OPTIONS,
                )}
                onChange={() =>
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="flex w-full items-center justify-end">
          <button className="close-button text-2xl" onClick={handleCancel}>
            &times;
          </button>
        </div>
            ...
            {currentPage < 3 ? (
              <button
                ...
                className="focus:shadow-outline w-24 rounded bg-blue-500
                                px-4 py-2 text-white hover:bg-blue-700 focus:outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50"
              >
                Next
              </button>
            ) : (
              <button
                ...
                className="rounded bg-blue-500 px-6 py-2 font-semibold text-white disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50"
              >
                Submit
              </button>
            )}
